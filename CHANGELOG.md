...
### Fixed
- **Static Signal timestamps in test mock hiding routing bugs**: The mock Signal API returned the same timestamp (`1234567890123`) for every send, causing all forwarded messages to share the same `SignalMsgID` in the database. This made quote-based routing tests indeterminate — `GetMessageMapping` would return an arbitrary row on hash collision. Fixed with an atomic counter generating unique timestamps per send.
- **Deploy script fails when base64 secrets contain slashes**: `sed` substitutions in `scripts/deploy.sh` used `/` as delimiter, which collided with `/` characters in base64-encoded secrets generated by `openssl rand -base64`. Changed to `|` delimiter.

### Added
- **WhatsApp send destination tracking in test infrastructure**: Mock API handlers now capture `chatId`, `body`, `replyTo`, and `mediaType` from every WhatsApp send request via `WhatsAppSendRecord`. Tests can now assert *where* messages were routed, not just *that* they were sent.
- **Unique WhatsApp response message IDs**: Mock `/sendText`, `/sendImage`, `/sendVideo`, `/sendVoice`, `/sendDocument` handlers now return unique `wamid.testN` IDs per send instead of static values.
- **Mock `/api/reaction` handler**: Added PUT handler for WhatsApp reaction endpoint, enabling end-to-end reaction routing tests.
- **`CreateTestMessageMappingWithChatID` helper**: Variant of `CreateTestMessageMapping` that accepts an explicit WhatsApp chat ID instead of deriving it from the session name.

### Testing
- **6 new quote routing integration tests** (`integration_test/quote_routing_test.go`):
  - `TestQuoteRouting_ReplyGoesToQuotedSender` — quotes Alice's message, verifies reply goes to Alice not Bob
  - `TestFallbackRouting_NoQuote_RoutesToLatestWithWarning` — unquoted reply goes to latest sender with Signal notification
  - `TestQuoteRouting_ExpiredMapping_ReturnsError` — quoting a non-existent message returns error, no WhatsApp send
  - `TestGroupQuoteRouting_CorrectGroup` — quotes Group A message, verifies reply goes to Group A not Group B
  - `TestSyncMessage_NestedQuote_DetectedCorrectly` — verifies fb39530 fix for nested `dataMessage.quote`
  - `TestReactionRouting_TargetsCorrectMessage` — reaction targets correct message across multiple senders
- **3 new quote parsing unit tests** (`pkg/signal/client_test.go`):
  - `TestQuotedMessageFromRestQuote` — nil, valid, and zero-ID quote conversion
  - `TestGetQuote_Priority` — verifies priority chain: `Quote` > `QuoteMsg` > `QuotedMsg` > `DataMessage.GetQuote()`
  - `TestConvertSyncMessageToSignalMessage_QuoteNesting` — all 4 quote nesting levels (top-level, quoteMessage, quotedMessage, nested dataMessage)
- **Enabled previously-skipped concurrent tests**: `TestConcurrentWhatsAppMessages` and `TestConcurrentMessagesFromDifferentSenders` now run with `IsolationStrict` (file-based SQLite) instead of being skipped.
