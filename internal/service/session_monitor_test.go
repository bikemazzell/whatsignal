...
// Mock ContainerRestarter for testing
type mockContainerRestarter struct {
	mock.Mock
}

func (m *mockContainerRestarter) RestartContainer(ctx context.Context) error {
	args := m.Called(ctx)
	return args.Error(0)
}

func TestSessionMonitor_FailureTracking(t *testing.T) {
	client := &mockWhatsAppClient{}
	logger := logrus.New()
	logger.SetLevel(logrus.ErrorLevel)

	client.On("GetSessionName").Return("test-session").Maybe()
	...
	monitor := NewSessionMonitorWithContainerRestart(client, logger, 30*time.Second, 30*time.Second, config)

	// Verify initial state
	assert.Equal(t, 0, monitor.consecutiveFailures)

	// Simulate first failure
	client.On("GetSessionStatus", mock.Anything).Return(&types.Session{Name: "test-session", Status: "STOPPED"}, nil).Once()
	client.On("RestartSession", mock.Anything).Return(assert.AnError).Once()
	...
	// Simulate successful restart - should reset counter
	client.On("GetSessionStatus", mock.Anything).Return(&types.Session{Name: "test-session", Status: "STOPPED"}, nil).Once()
	client.On("RestartSession", mock.Anything).Return(nil).Once()
	client.On("WaitForSessionReady", mock.Anything, mock.AnythingOfType("time.Duration")).Return(nil).Once()
	...
	failures = monitor.consecutiveFailures
	monitor.mu.Unlock()
	assert.Equal(t, 0, failures, "Consecutive failures should reset after successful restart")
}

func TestSessionMonitor_ContainerRestartThreshold(t *testing.T) {
	client := &mockWhatsAppClient{}
	logger := logrus.New()
	logger.SetLevel(logrus.ErrorLevel)
	mockRestarter := &mockContainerRestarter{}
	...
	monitor := NewSessionMonitorWithContainerRestart(client, logger, 30*time.Second, 30*time.Second, config)
	monitor.containerRestarter = mockRestarter

	ctx := context.Background()

	// Simulate 3 consecutive failures - should trigger container restart
	for i := 0; i < 3; i++ {
		client.On("GetSessionStatus", mock.Anything).Return(&types.Session{Name: "test-session", Status: "STOPPED"}, nil).Once()
		client.On("RestartSession", mock.Anything).Return(assert.AnError).Once()

		if i == 2 {
			// On the 3rd failure, expect container restart
			mockRestarter.On("RestartContainer", mock.Anything).Return(nil).Once()
		}

		monitor.checkAndRecoverSession(ctx)
	}

	mockRestarter.AssertExpectations(t)
	client.AssertExpectations(t)
}

func TestSessionMonitor_ContainerRestartCooldown(t *testing.T) {
	client := &mockWhatsAppClient{}
	logger := logrus.New()
	logger.SetLevel(logrus.ErrorLevel)
	mockRestarter := &mockContainerRestarter{}
	...
	// First set of failures - should trigger container restart
	for i := 0; i < 2; i++ {
		client.On("GetSessionStatus", mock.Anything).Return(&types.Session{Name: "test-session", Status: "STOPPED"}, nil).Once()
		client.On("RestartSession", mock.Anything).Return(assert.AnError).Once()
	...
	// Second set of failures immediately after - should NOT trigger container restart (cooldown)
	for i := 0; i < 2; i++ {
		client.On("GetSessionStatus", mock.Anything).Return(&types.Session{Name: "test-session", Status: "STOPPED"}, nil).Once()
		client.On("RestartSession", mock.Anything).Return(assert.AnError).Once()
	...
	mockRestarter.AssertNumberOfCalls(t, "RestartContainer", 1)
}

func TestSessionMonitor_DisabledContainerRestart(t *testing.T) {
	client := &mockWhatsAppClient{}
	logger := logrus.New()
	logger.SetLevel(logrus.ErrorLevel)
	...
	// Verify NoOpContainerRestarter is used
	_, isNoOp := monitor.containerRestarter.(*NoOpContainerRestarter)
	assert.True(t, isNoOp, "Should use NoOpContainerRestarter when feature is disabled")

	ctx := context.Background()

	// Simulate multiple failures - should track but not trigger container restart
	for i := 0; i < 3; i++ {
		client.On("GetSessionStatus", mock.Anything).Return(&types.Session{Name: "test-session", Status: "STOPPED"}, nil).Once()
		client.On("RestartSession", mock.Anything).Return(assert.AnError).Once()
