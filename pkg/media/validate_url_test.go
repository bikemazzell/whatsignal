...
func TestValidateDownloadURL_DockerInternalIP(t *testing.T) {
	...
	h := setupHandlerForURLValidation(t, "http://192.168.X.X:3000")
...
func TestValidateDownloadURL_DockerInternalHosts(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			url:         "http://192.168.X.X:3000/api/files/voice.ogg",
			expectError: false,
		},
		{
			name:        "WAHA host IP with different path - allowed",
			url:         "http://192.168.X.X:3000/media/image.jpg",
			expectError: false,
		},
	}
...
func TestValidateDownloadURL_PrivateIPAsWAHAHost(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wahaBaseURL: "http://192.168.X.X:3000",
			testURL:     "http://192.168.X.X:3000/api/files/test.jpg",
			expectError: false,
		},
		{
